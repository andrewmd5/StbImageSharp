...
namespace StbImageSharp
{
	...
	{
		internal static int stbi__gif_test(stbi__context s)
		{
		...
		internal static void* stbi__gif_load(stbi__context s, int* x, int* y, int* comp, int req_comp,
		...
		internal static void* stbi__load_gif_main(stbi__context s, int** delays, int* x, int* y, int* z, int* comp,
			int req_comp)
		{
		...
		internal static int stbi__gif_info(stbi__context s, int* x, int* y, int* comp)
		{
			return stbi__gif_info_raw(s, x, y, comp);
		}

		internal static int stbi__gif_test_raw(stbi__context s)
		{
			var sz = 0;
		...
		internal static void stbi__gif_parse_colortable(stbi__context s, byte[][] pal, int num_entries, int transp)
		{
			var i = 0;
		...
		internal static int stbi__gif_header(stbi__context s, stbi__gif g, int* comp, int is_info)
		{
			byte version = 0;
			if (stbi__get8(s) != 71 || stbi__get8(s) != 73 || stbi__get8(s) != 70 || stbi__get8(s) != 56)
		...
		internal static int stbi__gif_info_raw(stbi__context s, int* x, int* y, int* comp)
		{
			var g = new stbi__gif();
			if (g == null)
		...
		internal static void stbi__out_gif_code(stbi__gif g, ushort code)
		{
			byte* p;
			var idx = 0;
		...
		internal static byte* stbi__process_gif_raster(stbi__context s, stbi__gif g)
		{
			byte lzw_cs = 0;
			var len = 0;
			var init_code = 0;
		...
		internal static byte* stbi__gif_load_next(stbi__context s, stbi__gif g, int* comp, int req_comp, byte* two_back)
		{
			var dispose = 0;
			var first_frame = 0;
			var pi = 0;
		...
		internal static void* stbi__load_gif_main_outofmem(stbi__gif g, byte* _out_, int** delays)
		{
			CRuntime.free(g._out_);
			CRuntime.free(g.history);
			CRuntime.free(g.background);
		...
		internal class stbi__gif
		{
			internal byte* _out_;
			internal byte* background;
			internal int bgindex;
			internal stbi__gif_lzw[] codes = new stbi__gif_lzw[8192];
			internal byte[][] color_table;
			internal int cur_x;
			internal int cur_y;
			internal int delay;
			internal int eflags;
			...
			internal int lflags;
			internal int line_size;
			internal byte[][] lpal = Utility.CreateArray<byte>(256, 4);
			internal int max_x;
			internal int max_y;
			internal byte[][] pal = Utility.CreateArray<byte>(256, 4);
			internal int parse;
			internal int ratio;
			internal int start_x;
			internal int start_y;
			internal int step;
